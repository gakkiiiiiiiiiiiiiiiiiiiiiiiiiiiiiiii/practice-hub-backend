...
describe('AgentPricePolicyService', () => {
	const createService = (config: any = null) => {
		const systemConfigRepository = {
			findOne: jest.fn().mockResolvedValue(
				config
					? { id: 1, configKey: 'agent_price_exclusions', configValue: JSON.stringify(config) }
					: null,
			),
		...
		const courseRepository = { find: jest.fn().mockResolvedValue([]) };
		const appUserRepository = {
			findOne: jest.fn().mockResolvedValue({ id: 1, role: AppUserRole.ADMIN }),
		};
		const distributorRepository = { findOne: jest.fn().mockResolvedValue(null) };
	...
	it('uses the configured level price when the course is not excluded', async () => {
		...
		await expect(
			service.getCoursePrice(
				{ id: 12, category: '护理', sub_category: '内科', price: 20, agent_price: 6, agent_prices: { '2': 4 } } as any,
				2,
			),
	...
	it('uses the original price for a course in an excluded primary category', async () => {
		const { service, courseCategoryRepository } = createService({ category_ids: [3], package_section_ids: [] });
		courseCategoryRepository.find.mockResolvedValue([{ id: 3, name: '护理', parent_id: null }]);

		await expect(
			service.getCoursePrice(
				{ id: 12, category: '护理', sub_category: '内科', price: 20, agent_price: 6, agent_prices: { '3': 3 } } as any,
				3,
			),
		).resolves.toEqual({ unitPrice: 20, excluded: true, pricingMode: 'original' });
	});

	it('only excludes the matching child category', async () => {
		const { service, courseCategoryRepository } = createService({ category_ids: [8], package_section_ids: [] });
		...
		await expect(
			service.getCoursePrice(
				{ id: 13, category: '护理', sub_category: '外科', price: 20, agent_price: 6 } as any,
				1,
			),
	...
	it('normalizes duplicate ids when an admin updates the policy', async () => {
		const { service, courseCategoryRepository, packageSectionRepository, systemConfigRepository } = createService();
		courseCategoryRepository.count.mockResolvedValue(1);
		packageSectionRepository.count.mockResolvedValue(1);

		await service.updatePolicy(1, { category_ids: [3, 3], package_section_ids: [5, 5] });

		expect(systemConfigRepository.save).toHaveBeenCalledWith(
			expect.objectContaining({
				configValue: JSON.stringify({ category_ids: [3], package_section_ids: [5] }),
			}),
		);
	});
	...
	it('rejects policy updates from a non-admin user', async () => {
		...
		appUserRepository.findOne.mockResolvedValue({ id: 9, role: AppUserRole.USER });

		await expect(
			service.updatePolicy(9, { category_ids: [3, 3], package_section_ids: [5] }),
