...
describe("UploadService storage provider credentials", () => {
  ...
  it("uses WeChat cloud upload credentials for Tencent COS", async () => {
    ...
    const credentials = await service.getPostUploadCredentials(
      "course-files/staging/test.pdf",
      "application/pdf",
    );
    ...
    expect(credentials).toMatchObject({
      ...
      finalFileUrl:
        "https://example-cos-bucket.tcb.qcloud.la/course-files/staging/test.pdf",
    });
  });
  ...
  it("converts a WeChat cloud file ID to its TCB HTTPS URL", () => {
    const fileId =
      "cloud://example-env.example-cos-bucket/feedback/example.jpg";

    expect(service.getPublicImageUrl(fileId)).toBe(
      "https://example-cos-bucket.tcb.qcloud.la/feedback/example.jpg",
    );
  });
