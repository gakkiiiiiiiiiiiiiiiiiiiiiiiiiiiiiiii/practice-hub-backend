...
export class UploadService {
  ...
  constructor(
    ...
    this.cosRegion = this.configService.get<string>(
      "COS_REGION",
      "ap-shanghai",
    );
    ...
    if (this.bucket && accessKeyId && accessKeySecret) {
      ...
      this.logger.log(
        `阿里云 OSS 已启用: bucket=${this.bucket}, region=${this.region}`,
      );
    } else {
    ...
    this.cos = new COS({
      getAuthorization: async (_options, callback) => {
        try {
          ...
          this.logger.error(
            `获取腾讯云 COS 临时密钥失败: ${error?.message || error}`,
          );
          callback({
            TmpSecretId: "",
            TmpSecretKey: "",
  ...
  private async getCosFileMetaData(
    cloudPath: string,
    openid = "",
  ): Promise<string | null> {
    ...
    try {
      ...
      return data.errcode === 0
        ? data.respdata?.x_cos_meta_field_strs?.[0] || null
        : null;
    } catch (error: any) {
      this.logger.warn(
        `获取 COS 文件元数据失败，将继续上传: ${error?.message || error}`,
      );
      return null;
    }
  }
  ...
  private isValidFolder(folder: string): boolean {
    const parts = String(folder || "")
      ...
      .filter(Boolean);
    return (
      parts.length > 0 && parts.every((part) => this.isValidFilename(part))
    );
  }
  ...
  private async uploadToCos(
    ...
    if (
      !this.isValidFilename(file.originalname) ||
      !this.isValidFolder(folder)
    ) {
  ...
  async deleteByUrlOrThrow(
    ...
    if (fileUrl.includes("/uploads/")) {
      const relativePath = decodeURIComponent(
        fileUrl.split("/uploads/")[1].split(/[?#]/, 1)[0],
      ).replace(/^\/+/, "");
  ...
  async promoteCourseFileUrl(
    fileUrl: string,
  ): Promise<{ url: string; promoted: boolean }> {
    ...
    if (fileUrl.includes("/uploads/")) {
      ...
      if (!relativePath.startsWith(stagingPrefix))
        return { url: fileUrl, promoted: false };
      ...
      if (!fs.existsSync(sourcePath))
        throw new BadRequestException("待绑定的课程文件不存在");
    ...
    if (!sourceKey || !sourceKey.startsWith(stagingPrefix))
      return { url: fileUrl, promoted: false };
  ...
  async deleteCoursePreviewPrefix(prefix: string): Promise<number> {
    ...
    if (this.oss) {
      ...
      do {
        const result: any = await this.oss.list(
          { prefix: safePrefix, marker, "max-keys": 1000 },
          {},
        );
    ...
    this.logger.log(
      `[OSS删除] 课程预览缓存清理完成: prefix=${safePrefix}, count=${deletedCount}`,
    );
    return deletedCount;
  }
  ...
  private async uploadCourseFileToLocal(
    file: Express.Multer.File,
    folder: string,
  ): Promise<string> {
    const allowedExts = [".pdf", ".doc", ".docx"];
    const relativePath = await this.saveFileToLocalWithExts(
      file,
      folder,
      allowedExts,
    );
  ...
  private getPdfBuffer(file: Express.Multer.File): Promise<Buffer> {
    ...
    if (f.buffer && f.buffer.length > 0) {
      return Promise.resolve(
        Buffer.isBuffer(f.buffer) ? f.buffer : Buffer.from(f.buffer),
      );
    }
    if (file.path && fs.existsSync(file.path)) {
  ...
  async uploadBufferToOss(
    ...
    const safeKey = this.normalizeObjectKey(key);
    if (
      (await this.getStorageProvider()) === StorageProvider.COS &&
      this.isWeChatCloudBase()
    ) {
  ...
  getPublicImageUrl(url: string): string {
    ...
    if (bucket !== this.legacyCosBucket) {
  ...
  async saveCourseFileChunk(
    ...
    const resolvedUploadDir = path.resolve(this.uploadDir);
    if (
      !resolvedDir.startsWith(resolvedUploadDir) ||
      resolvedDir === resolvedUploadDir
    ) {
  ...
  isAllowedProxyUrl(url: string): boolean {
    ...
    try {
      ...
      if (this.legacyCosBucket) {
        ...
        allowedHosts.add(
          `${this.legacyCosBucket}.cos.${this.cosRegion}.myqcloud.com`,
        );
      }
  ...
  private extractLegacyCloudKey(url: string): string | null {
    ...
    try {
      const key = decodeURIComponent(
        url.slice(prefix.length).split(/[?#]/, 1)[0],
      ).replace(/^\/+/, "");
  ...
  async proxyImage(
    ...
    try {
      if (this.isTencentStorageUrl(publicUrl)) {
      ...
      const data = Buffer.isBuffer(result.content)
        ? result.content
        : Buffer.from(result.content as any);
      const contentType =
        (result.res?.headers?.["content-type"] as string) ||
        this.sniffImageMime(data) ||
        "image/png";
      return { data, contentType };
    } catch (error: any) {
      const fallbackUrl = this.getLegacyCosFallbackUrl(publicUrl, key);
      if (fallbackUrl) {
        try {
  ...
  private isTencentStorageUrl(url: string): boolean {
    return (
      Boolean(this.extractLegacyCloudKey(url)) || this.isLegacyCosHttpsUrl(url)
    );
  }
