...
export class UploadService {
	...
	private async uploadToCos(file: Express.Multer.File, folder = 'images', openid = ''): Promise<string> {
	...
	private getCosObjectUrl(key: string): string {
		const safeKey = this.normalizeObjectKey(key).split('/').map(encodeURIComponent).join('/');
	...
	private async uploadCourseFileToCos(file: Express.Multer.File, folder: string, openid: string): Promise<string> {
		const ext = this.getFileExtension(file.originalname).toLowerCase();
		...
		await this.putObjectToCos(key, buffer || fs.createReadStream(filePath), file.mimetype, size, openid);
		return this.getCosObjectUrl(key);
	}
	...
	async proxyImage(url: string): Promise<{ data: Buffer; contentType: string }> {
		...
		try {
			...
			throw new BadRequestException(`拉取对象存储图片失败: ${error?.message || '未知错误'}`);
		}
	}
